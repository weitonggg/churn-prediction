...
def data_preprocess(df):
    ...
    online_security_dummy = pd.get_dummies(df.OnlineSecurity).rename(columns = lambda x: 'onlinesecurity_' + str(x)).astype(object)
    online_backup_dummy = pd.get_dummies(df.OnlineBackup).rename(columns = lambda x: 'onlinebackup_' + str(x)).astype(object)
    device_protect_dummy = pd.get_dummies(df.DeviceProtection).rename(columns = lambda x: 'deviceprotect_' + str(x)).astype(object)
